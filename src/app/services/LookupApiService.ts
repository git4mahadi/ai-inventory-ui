...
export class LookupApiService {
  ...
  createLookup(data: LookupDto): Observable<LookupResponse> {
    return this.http
      .post<ApiResponse<LookupResponse>>(this.baseUrl, data)
      .pipe(
        unwrapApiData(),
  ...
  updateLookup(id: string, data: LookupDto): Observable<LookupResponse> {
    return this.http
      .put<ApiResponse<LookupResponse>>(`${this.baseUrl}/${id}`, data)
      .pipe(
        unwrapApiData(),
        map((result) => this.requireLookup(result)),
        catchError((err: { error?: { message?: string } }) => {
  ...
  deleteLookup(id: string): Observable<LookupResponse | null> {
    return this.http
      .delete<ApiResponse<LookupResponse | null>>(`${this.baseUrl}/${id}`)
      .pipe(
        unwrapApiData(),
        catchError((err: { error?: { message?: string } }) => {
  ...
  getLookupById(id: string): Observable<LookupResponse> {
    return this.http
      .get<ApiResponse<LookupResponse>>(`${this.baseUrl}/${id}`)
      .pipe(
        unwrapApiData(),
        map((result) => this.requireLookup(result)),
        catchError((err: { error?: { message?: string } }) => {
  ...
  getLookupListByEnumKey(lookupEnumKey: string): Observable<LookupResponse[]> {
    return this.http
      .get<ApiResponse<LookupResponse[]>>(
        `${this.baseUrl}/lookup-enum/${lookupEnumKey}`,
      )
      .pipe(
        unwrapApiData(),
        catchError((err: { error?: { message?: string } }) => {
  ...
  getLookupListByKeys(
    keys: string[],
  ): Observable<Record<string, LookupResponse[]>> {
    return this.http
      .post<ApiResponse<Record<string, LookupResponse[]>>>(
        `${this.baseUrl}/lookup-enum-keys`,
        keys,
      )
      .pipe(
        unwrapApiData(),
  ...
  searchPage(data: LookupSearchDto): Observable<Page<LookupResponse>> {
    return this.http
      .post<ApiResponse<Page<LookupResponse>>>(
  ...
  searchList(data: LookupSearchDto): Observable<LookupResponse[]> {
    return this.http
      .post<ApiResponse<LookupResponse[]>>(`${this.baseUrl}/search-list`, data)
      .pipe(
        unwrapApiData(),
        catchError((err: { error?: { message?: string } }) => {
          this.toast.error(err?.error?.message || 'Failed to load lookups');
  ...
  searchTerm(data: LookupSearchDto): Observable<LookupResponse[]> {
    return this.http
      .post<ApiResponse<LookupResponse[]>>(`${this.baseUrl}/search-term`, data)
      .pipe(
        unwrapApiData(),
        catchError((err: { error?: { message?: string } }) => {
          this.toast.error(err?.error?.message || 'Failed to search lookups');
